...
- Branch `test/bucket-e-bonifica` (2026-07-14, da main post-merge): (a) nuovo `recording_bloc_waveform_buckets_test.dart` — 7 test sui bucket nativi: accumulo, append, applicazione in pausa, dedup batch stale, cap 3000 con drop dalla testa, ignore su stato non-recording, wiring stream→stato dopo StartRecording; (b) bonifica teatro: geolocation da 29 test finti a 5 onesti (scoperto contratto: `getCurrentAddress` rilancia `SystemException`, il fallback è in `getRecordingLocationName`; refactor wrapper iniettabile segnato in tech-debt), eliminate le 6 guardie if-found nei widget test (ora assertive, con finder FaIcon per lo skull), aggiunti `verify()`/`verifyNever()` sulle interazioni in folder/recording bloc test. Suite: **262 verdi, 0 falliti** (−24 test finti geolocation, +7 bucket), analyzer pulito. Resta: test lifecycle post validazione device.
