...
- Analisi 2026-07-15: divergenza registrazione iOS vs macOS, vedi `analysis/2026-07-15-divergenza-recording-ios-macos.md`. Il plugin macOS è un fork mai riallineato: manca `getRecordingStatus` (→ desync stato al ritorno focus finestra, il reconcile Dart degrada a `idle()`), mancano i bucket waveform nativi (→ `waveformAmplitudeSamples` sempre vuoto su macOS), posizione non cumulativa dopo pause/resume (manca `framesInPreviousSegments`), `initialElapsedMs` ignorato, zero interruption/lifecycle handling.
- Fix 2026-07-15 su branch `fix/macos-recording-status-frames` (confermato utente): applicati i fix 1 e 3 dell'analisi a `macos/Runner/AudioEnginePlugin.swift` — aggiunti `framesInPreviousSegments`/`framesWrittenThisSegment`/`outputSampleRate` come su iOS (tick cumulativo attraverso pause/resume, reset in start/stop/cancel, accumulo in pause, reset segmento in resume) e implementato `getRecordingStatus` con lo stesso payload di iOS. Build macOS debug verde. Committato in `3dcfe9d`. Aggiunti test contratto Dart↔nativo in `test/unit/services/audio_engine_service_recording_status_test.dart` (10 test): parsing payload `getRecordingStatus` (incl. path NSNull e clamp amplitude), sync flag interni quando il nativo risponde, degrado a idle col vecchio plugin (documenta il desync pre-fix), reconcile `syncRecordingStatusFromNative` (attiva/pausa/idle), tick cumulativi monotoni post-resume. Suite completa 272 verdi. Nota: sono test del contratto lato Dart — la logica Swift resta senza XCTest (gap noto). Restano aperti su macOS: bucket waveform nativi (fix 2), `initialElapsedMs` (fix 4), interruption/device-change handling (fix 5).
...
- Follow-up 2026-07-14 (decisioni utente): creato gate CI `.github/workflows/ci.yml` (analyze con warning/info non fatali + flutter test, Flutter 3.35.5); rimossi eventi morti `FilterFolders`/`SortFolders` + enum `FolderSortType` da folder_event.dart e relativi test skip; rimossi i 2 test integration skip — deciso che la MainScreen NON mostra RecordingError (gli errori vivono nel bottom sheet/lista). Suite: 279 verdi, 0 falliti, 0 skip.
- **MERGED in main** (PR #1, merge `c6ccea4`, 2026-07-14): risanamento + gate CI. Primo giro CI verde in 2m07s. Da ora ogni push su main e ogni PR passano dal gate.
- **MERGED in main** (PR #2, merge `12b2d78`, 2026-07-14): (a) nuovo `recording_bloc_waveform_buckets_test.dart` — 7 test sui bucket nativi: accumulo, append, applicazione in pausa, dedup batch stale, cap 3000 con drop dalla testa, ignore su stato non-recording, wiring stream→stato dopo StartRecording; (b) bonifica teatro: geolocation da 29 test finti a 5 onesti (scoperto contratto: `getCurrentAddress` rilancia `SystemException`, il fallback è in `getRecordingLocationName`; refactor wrapper iniettabile segnato in tech-debt), eliminate le 6 guardie if-found nei widget test (ora assertive, con finder FaIcon per lo skull), aggiunti `verify()`/`verifyNever()` sulle interazioni in folder/recording bloc test. Gate CI verde in 1m20s. Suite su main: **262 verdi, 0 falliti**, analyzer pulito. Resta lato test: solo lifecycle post validazione device.
