...
- Fix 2026-07-15 su branch `fix/macos-recording-status-frames` (confermato utente): applicati i fix 1 e 3 dell'analisi a `macos/Runner/AudioEnginePlugin.swift` — aggiunti `framesInPreviousSegments`/`framesWrittenThisSegment`/`outputSampleRate` come su iOS (tick cumulativo attraverso pause/resume, reset in start/stop/cancel, accumulo in pause, reset segmento in resume) e implementato `getRecordingStatus` con lo stesso payload di iOS. Build macOS debug verde. Restano aperti su macOS: bucket waveform nativi (fix 2), `initialElapsedMs` (fix 4), interruption/device-change handling (fix 5). NON committato.
